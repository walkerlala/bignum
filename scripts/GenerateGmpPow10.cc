...
class GmpPow10StrArray {
   ...
   private:
    std::array<std::array<char, kDecimalMaxNumPow10 + 1>, kDecimalMaxNumPow10 + 1> m_interpreted;
};
   ...
   private:
    GmpPow10Array() {
        for (size_t i = 0; i <= kDecimalMaxNumPow10; i++) {
            ...
            int64_t xsize =
                mpn_set_str(wrapper.mpz._mp_d, (unsigned char *)sv.data(), sv.size(), /*base*/ 10);
            assert(xsize >= 0);
...
int main() {
    ...
    for (size_t i = 0; i <= kDecimalMaxNumPow10; i++) {
        ...
        std::cout << i << ", GmpWrapper(" << wrapper->mpz._mp_size << ", 0x" << std::hex
                  << wrapper->limbs[0] << ", 0x" << wrapper->limbs[1] << ", 0x" << wrapper->limbs[2]
                  << ", 0x" << wrapper->limbs[3] << ", 0x" << wrapper->limbs[4] << ")," << std::dec
                  << std::endl;
    }

    // gmp min value and max value, precision 65
    const char *original_max_val_str =
        "99999999999999999999999999999999999999999999999999999999999999999";
    char max_val_str[66] = {0};
    for (int i = 0; i < 65; i++) {
        max_val_str[i] = gmp_digit_value_tab[static_cast<int>(original_max_val_str[i])];
    }

    GmpWrapper max_val;
    int64_t xsize = mpn_set_str(max_val.mpz._mp_d, (unsigned char *)max_val_str, 65, /*base*/ 10);
    assert(xsize >= 0);
    ...
    std::cout << "Max_GmpWrapper(" << max_val.mpz._mp_size << ", 0x" << std::hex << max_val.limbs[0]
              << ", 0x" << max_val.limbs[1] << ", 0x" << max_val.limbs[2] << ", 0x"
              << max_val.limbs[3] << ", 0x" << max_val.limbs[4] << ")," << std::dec << std::endl;

    std::cout << "Min_GmpWrapper(" << min_val.mpz._mp_size << ", 0x" << std::hex << min_val.limbs[0]
              << ", 0x" << min_val.limbs[1] << ", 0x" << min_val.limbs[2] << ", 0x"
              << min_val.limbs[3] << ", 0x" << min_val.limbs[4] << ")," << std::dec << std::endl;
    ...
    std::cout << "Minus_1_GmpWrapper(" << minus_one.mpz._mp_size << ", 0x" << std::hex
              << minus_one.limbs[0] << ", 0x" << minus_one.limbs[1] << ", 0x" << minus_one.limbs[2]
              << ", 0x" << minus_one.limbs[3] << ", 0x" << minus_one.limbs[4] << ")," << std::dec
              << std::endl;
