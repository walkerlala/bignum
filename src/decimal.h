...
template <typename T>
constexpr inline double DecimalImpl<T>::to_double() const {
        int scale = m_scale;
...
template <typename T>
constexpr inline bool DecimalImpl<T>::to_bool() const {
        if (m_dtype == DType::kInt64) {
                return (m_i64 != 0);
        } else if (m_dtype == DType::kInt128) {
                return (m_i128 != 0);
...
template <typename T>
constexpr inline ErrCode DecimalImpl<T>::to_int64(int64_t &i) const {
        ErrCode err = kSuccess;
        if (m_dtype == DType::kInt64) {
...
template <typename T>
constexpr inline ErrCode DecimalImpl<T>::to_int128(__int128_t &i) const {
        ErrCode err = kSuccess;
        if (m_dtype == DType::kInt64) {
